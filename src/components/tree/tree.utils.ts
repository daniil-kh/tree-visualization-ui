...
export const drawBNode = <T>(
  ...
  ctx.arc(node.x + adj, node.y, circleRadius, 0, 2 * Math.PI);
  ctx.stroke();
  //ctx.strokeStyle = "#000000";
  ctx.font = "20px Arial";
  ctx.textAlign = "center";
  ctx.fillText((node.data as unknown) as string, node.x + adj, node.y + 7);
  if (node.parent) {
    ...
    ctx.moveTo(
      node.parent.x + adj + (node.x > node.parent.x ? X : -X),
      node.parent.y + Y
    );
    ctx.lineTo(node.x + adj + (node.x > node.parent.x ? -X : X), node.y - Y);
    ctx.stroke();
  }
  ctx.strokeStyle = "#000000";
};
